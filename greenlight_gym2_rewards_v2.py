...
# Named indices into the GreenLight parameter vector (208 entries).
# Source: gl_gym/configs/greenlight_parameters.py (TOMATO_PARAMETER_DEFS)
_P_FLOOR_AREA = 46         # aFlr  [m^2]
_P_MAX_HEATING = 108       # phiPipeMax  [W]
_P_MAX_CO2 = 109           # phiExtCo2Max  [mg/s]
_P_MAX_FRUIT_GROWTH = 154  # rgFruit  [mg CH2O / m^2 / s]
_P_LAMP_POWER = 172        # thetaLampMax  [W/m^2]


@dataclass
class _BoundCtx:
    """Minimal context for price-model calls during normalization bound computation."""
    p: np.ndarray
    dt: float
...
class GreenhouseReward(BaseReward):
    ...
    def __init__(
        ...
        self.pen_yield = pen_yield
        self.min_yield_frac = min_yield_frac
        self._init_costs()
        self._init_violations()

        # Lightweight context sufficient for price-model calls at init time
        _ctx = _BoundCtx(p=p, dt=dt)
        self.max_profit = self.max_profit_reward(_ctx)
        self.min_profit = self.min_profit_reward(_ctx)
    ...
    def max_profit_reward(self, ctx: RewardContext) -> float:
        ...
        """
        max_gains = ctx.p[_P_MAX_FRUIT_GROWTH] * ctx.dt * 1e-6 / self.dmfm * self.fruit_price_model.get_price(ctx)
        return max_gains
    ...
    def min_profit_reward(self, ctx: RewardContext) -> float:
        ...
        """
        max_heating = ctx.p[_P_MAX_HEATING] / ctx.p[_P_FLOOR_AREA] * ctx.dt/3600*1e-3 * self.heating_price_model.get_price(ctx)  # W → kWh/m²
        max_elec = ctx.p[_P_LAMP_POWER] * ctx.dt/3600*1e-3 * self.elec_price_model.get_price(ctx)                               # W/m² → kWh/m²
        max_co2 = ctx.p[_P_MAX_CO2] / ctx.p[_P_FLOOR_AREA] * ctx.dt * 1e-6 * self.co2_price_model.get_price(ctx)               # mg/s → kg/m²
        return -(max_heating + max_elec + max_co2)
    ...
    def _variable_costs(self, ctx: RewardContext) -> tuple[float, dict[str, float]]:
        ...
        """
        heating_energy = ctx.u[0] * ctx.p[_P_MAX_HEATING] / ctx.p[_P_FLOOR_AREA] * ctx.dt/3600 * 1e-3  # W → kWh/m²
        elec_use = ctx.u[4] * ctx.p[_P_LAMP_POWER] * ctx.dt/3600*1e-3                                 # W/m² → kWh/m²
        co2_dosing = ctx.u[1] * ctx.p[_P_MAX_CO2] / ctx.p[_P_FLOOR_AREA] * ctx.dt * 1e-6             # mg/s → kg/m²
    ...
    def _yield_penalty(self, ctx: RewardContext) -> tuple[float, dict[str, float]]:
        """Penalise steps where fruit growth rate falls below min_yield_frac * max possible.

        Prevents reward-hacking strategies that sacrifice yield to minimise energy costs.
        Shortfall is normalised to [0, 1] so pen_yield controls the scale directly.
        """
        if self.pen_yield == 0.0:
            return 0.0, {"yield_penalty": 0.0}
        max_growth = ctx.p[_P_MAX_FRUIT_GROWTH] * ctx.dt          # mg/m²/step (theoretical max)
        actual_growth = max(0.0, float(ctx.x[25] - ctx.x_prev[25]))
        min_growth = self.min_yield_frac * max_growth
        shortfall = max(0.0, min_growth - actual_growth) / max_growth  # 0–1
        penalty = self.pen_yield * shortfall
        return penalty, {"yield_penalty": penalty}
    ...
    def compute_reward(self, ctx: RewardContext) -> SupportsFloat:
        ...
        yield_penalty, yield_penalty_info = self._yield_penalty(ctx)
        ...
        reward = scaled_profit - sum(scaled_pen) - control_penalty - yield_penalty
        info = {
            "reward": reward,
            "profit": profit,
